...
- [ ] [StyleSDF:High-Resolution 3D-Consistent Image and Geometry Generation](http://arxiv.org/pdf/2112.11427)仅在单视图 RGB 数据上进行训练, 使用 styleGAN2 生成图像,基于 SDF 的 3D 建模定义了详细的 3D 表面,从而实现一致的体积渲染(Baseline,也是子网络摸扩的 backbone, 使用 SDF 作为隐式几何表示)
